...
class QGDataset(Dataset):
    def __init__(
        ...
        # tokenizer: PreTrainedTokenizerFast(tokenizer_file="/home/wowns/data/KoreanHistoryProject/vocab/vocab.json"),
        ...
        # self.sos_token = tokenizer.encode("<s>")
...
class QGDecodingDataset(QGDataset):
    def __getitem__(self, index: int) -> GPTDecodingInputType:
        ...
        context_tokens = self.tokenizer.encode(f"문맥:{example.context}").ids
        answer_tokens = self.tokenizer.encode(f"답변:{example.answer}").ids

        # [SOS] + CONTEXT + ANSWER + 질문:
        conditional_tokens_len = 1 + len(context_tokens) + len(answer_tokens) + len(self.question_prefix_tokens)
        # ANSWER_SEQ + [EOS]
...
def load_as_dataset(dataset_path: str) -> List[QAExample]:
    with open(dataset_path) as f:
        as_raw_data = json.load(f)['data']

    examples = []
    
    for data in as_raw_data:
        example = QAExample(data['doc'], data['answer'])
        examples.append(example)
