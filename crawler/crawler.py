...
if __package__ is None:
    current_path = path.dirname(path.dirname(path.abspath(
        '/home/fhdufhdu/vscode/Project/KoreanHistoryProject/crawler/crawler.py')))
    sys.path.append(current_path)
    from util.func import load_json, save_json
else:
    from ..util.func import load_json, save_json
    ...
    while size_cnt < size:
        ...
        try:
            ...
            for cont_cnt in range(len(content_headers)):
                ...
                content = ''
                for cont_inner_cnt in range(len(contents)):
                    content += contents[cont_inner_cnt].get_text().strip() + '\n'
