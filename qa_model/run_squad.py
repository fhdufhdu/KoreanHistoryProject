...
ALL_MODELS = sum(
    (tuple(conf.pretrained_config_archive_map.keys()) for conf in (
        BertConfig, DistilBertConfig, RobertaConfig, XLNetConfig, XLMConfig)),
    (),
)
                    ...
                    if args.local_rank == -1 and args.evaluate_during_training:
                        ...
                        with open("eval_result_{}_{}.txt".format(list(filter(None, args.model_name_or_path.split("/"))).pop(),
                            ...
                            for key in sorted(official_eval_results.keys()):
                                logger.info("  %s = %s", key, str(
                                    official_eval_results[key]))
                                f.write(" {} = {}\n".format(
                                    key, str(official_eval_results[key])))
...
def main():
    ...
    # tokenizer = BertTokenizer(vocab_file='/home/fhdufhdu/vscode/KoreanHistoryProject/QAmodel/models_old/vocab.txt',
    #                           do_lower_case=False,
    #                           unk_token="[UNK]",
    #                           sep_token="[SEP]",
    #                           pad_token="[PAD]",
